...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| beyla.application | bool | `true` | Enable application observability for Beyla. Required for Application Observability. |
| beyla.debug | bool | `false` | Enable debug mode for Beyla |
| beyla.enabled | bool | `false` | Enable Beyla for automatic instrumentation eBPF in the cluster. |
| beyla.metrics | bool | `true` | Export Prometheus metrics. |
| beyla.namespaces | list | `[]` | Which namespaces to look to instrument services in Deployments, DaemonSets and StatefulSets. |
| beyla.network | bool | `false` | Enable network observability for Beyla. Required for Asserts. |
| beyla.process | bool | `false` | Enable process performance signals from instrumented services. |
| beyla.traces | bool | `false` | Export OTEL traces. |
