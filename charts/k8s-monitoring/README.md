...
| alloy-operator.fleetManagement.enabled | bool | `false` | Sync collectors with Fleet Management. Connects with `cluster.name` and the remoteConfig settings of the first collector that has remote config enabled, usually from `collectorCommon.alloy.remoteConfig`. Needs the Alloy Operator image from grafana-experimental/hackathon-18-remote-manager, set with `alloy-operator.image`. |
