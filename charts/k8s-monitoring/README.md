...
![Version: 4.0.2](https://img.shields.io/badge/Version-4.0.2-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 4.0.2](https://img.shields.io/badge/AppVersion-4.0.2-informational?style=flat-square)
...
destinations: # Map of destinations where telemetry data will be sent
  myPrometheus:
    type: prometheus
    url: https://prometheus.example.com/api/prom/push
...
from workloads on the cluster. Collectors are defined as a map under the `collectors` key, where each key is the name
of the collector.

To define collectors, add entries to the `collectors` map in your values file. Example:
...
**Specific features require specific collector configuration**. For example, the applicationObservability feature requires a collector with receiver ports open for select protocols. Check [individual feature documentation](./docs/Features.md) to find out about collector requirements.
...
| global.minScrapeInterval | string | "" (disabled) | When set, enforces a minimum scrape interval across all chart-managed scrape configurations. Any scrape interval (global, per-feature, or per-job) that is lower than this value will be clamped up to this value.  For annotation-based autodiscovery targets (k8s.grafana.com/metrics.scrapeInterval), enabling this setting will override the annotation value entirely with this floor value, regardless of whether the annotation specifies a higher or lower interval. This is because duration comparison is not possible at runtime in Alloy relabeling rules. If you need per-target scrape intervals that differ from the floor, leave this setting unset and manage intervals through per-feature configuration.  Applies to: all chart-templated scrape blocks, annotation-based autodiscovery, and Prometheus Operator object defaults. Does NOT apply to: extraConfig (raw Alloy config). |
