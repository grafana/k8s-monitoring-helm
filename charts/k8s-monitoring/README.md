...
And one of two options for telemetry collection:
...
| logs.cluster_events.namespaces | list | `[]` | Limit Kubernetes cluter events to these namespaces. |
...
If you would like to use the OpenTelemetry Collector instead of the Grafana Agent for capturing infrastructure
telemetry, you only have to make the following changes to your values file:
...
This will disable the deployment of the Grafana Agent for metrics and logs and enable the OpenTelemetry Collector for
metrics and logs. The appropriate ConfigMaps will be generated and attached to the right collector.
