...
{{- $k8sManifestTail := index $.Values.telemetryServices "k8s-manifest-tail" }}
{{- if not (dig "deploy" false $k8sManifestTail) }}
  {{- $msg := list "" (printf "The %s feature requires k8s-manifest-tail to be deployed." $featureName) }}
  {{- $msg = append $msg "Please set:" }}
  {{- $msg = append $msg "telemetryServices:" }}
  {{- $msg = append $msg "  k8s-manifest-tail:" }}
