...
-   **`run-test.sh`** — orchestrator (CI runner / laptop; invoked by `make run-test`). It:
  ...
  1.  builds a code bundle (repository + `helm-chart-toolbox`, no secrets);
  ...
  6.  deletes the VM (always, even on error).
...
-   **`run-on-vm.sh`** — worker (bundled, runs on the VM). It reads the secrets from `tmpfs`, installs
  ...
  `openshift-install create cluster` and `helm-test`, and tears down the cluster on exit.
...
The cluster is destroyed when `run-on-vm.sh` exits (retrying until it succeeds); the worker VM is
deleted by the orchestrator's cleanup. Both run on success and on error. As a backstop against an
orchestrator that exits early, the VM is created with `--max-run-duration=7200s --instance-termination-action=DELETE`,
