...
*   Add opt-in `applicationObservability.processors.transform.setServiceAttributesFromKubernetes` (default `false`): fills missing or empty `service.name` (from the `app.kubernetes.io/name` pod label, workload owner names, or pod name) and `service.namespace` (from the namespace name) on metrics, logs, and traces in the Application Observability pipeline.
    When enabled, this fallback takes precedence over the OTLP destination `logToResource`/`datapointToResource` promotion (#2651, part of #2547) (@jmichalek132)
