...
*   Add opt-in `applicationObservability.processors.transform.setServiceAttributesFromKubernetes` (default `false`): fills missing or empty `service.name` (from the `app.kubernetes.io/name` pod label, then workload owner names, then pod name) and `service.namespace` (from the namespace name) resource attributes on metrics, logs, and traces in the Application Observability pipeline. When enabled, the receiver-side fallback takes precedence over the OTLP destination `logToResource`/`datapointToResource` promotion of log-record or metric-datapoint scoped `service_name` attributes (#2651, part of #2547) (@jmichalek132)
*   Fix the Pod Logs via OpenTelemetry `service.name` fallback never using the `app.kubernetes.io/name` pod label: the chain read attribute `app.kubernetes.io/name` while the label is extracted as `app_kubernetes_io_name`. The fallback now reads whatever attribute `podLogsViaOpenTelemetry.labels` maps to the `app.kubernetes.io/name` label, so file-gathered pod logs now prefer that label over the deployment name (#2651) (@jmichalek132)
*   Remove the unreachable `k8s.replicaset.name` and `k8s.container.name` rules from the Pod Logs via OpenTelemetry `service.name` fallback (`k8s.replicaset.name` is never extracted by the feature's `k8sattributes` configuration, and `k8s.pod.name` is always set before the container rule can fire) (#2651) (@jmichalek132)
