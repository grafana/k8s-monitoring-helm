...
*   Pod Logs (via Loki and via Kubernetes API): stop indexing `service_instance_id` as a Loki stream label by default, since its value is unique per pod and unbounded over pod churn. It remains available as structured metadata. To restore the previous behavior, remove `service.instance.id` from `structuredMetadata`. (#3051)
