...
*   **Behavior change:** default `openTelemetryConversion.keepIdentifyingResourceAttributes` to `true` for Prometheus destinations. The `target_info` metric now retains `service_name`, `service_namespace`, and `service_instance_id` labels during OTLP to Prometheus conversion, which increases `target_info` cardinality and may affect queries that join on it. Set `openTelemetryConversion.keepIdentifyingResourceAttributes: false` on the destination to restore the previous behavior. (#2722) (@rlankfo)
