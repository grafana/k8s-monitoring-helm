...
The following is the list of images potentially used in the 2.0.46 version of the k8s-monitoring Helm chart:

| Image Name | Repository | Feature |
| ---------- | ---------- | ------- |
| Alloy | docker.io/grafana/alloy:v1.10.1 | Always used. The telemetry data collector. |
| Beyla | docker.io/grafana/beyla:2.6.3 | Automatically instruments apps on the cluster, generating metrics and traces. Enabled with `autoInstrumentation.beyla.enabled=true`. |
...
| kube-state-metrics | registry.k8s.io/kube-state-metrics/kube-state-metrics:v2.17.0 | Gathers Kubernetes cluster object metrics. Enabled with `clusterMetrics.kube-state-metrics.deploy=true`. |
...
| Windows Exporter | ghcr.io/prometheus-community/windows-exporter:0.31.3 | Gathers Kubernetes cluster node metrics for Windows nodes. Enabled with `clusterMetrics.windows-exporter.deploy=true`. |
