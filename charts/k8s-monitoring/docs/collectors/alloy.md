...
2.  The settings in the `collectorCommon.alloy` section of the values.yaml file, which apply to all Alloy instances.
3.  The settings from any [presets](https://github.com/grafana/k8s-monitoring-helm/tree/main/charts/k8s-monitoring/docs/collectors/presets/) applied to the collector. When multiple presets are set, later ones overwrite earlier ones.
