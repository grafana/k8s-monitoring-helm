...
# Dependent charts use two conventions for global image settings, so we define it both ways.
global:
  # Nested convention: applies to Alloy, the Alloy Operator, the Alloy removal hooks, and Beyla.
  image:
  ...
  # Flat convention: registry and pull secrets for kube-state-metrics, Node Exporter, and Windows Exporter.
  # These charts do not honor a global image pull policy; set image.pullPolicy per-service if you need one.
