...
A router declares an `ecosystem` -- here `opentelemetry`, because `podLogsViaOpenTelemetry` collects
via the OpenTelemetry Protocol -- which means its conditions match on resource attributes. In this
example, the router is named `tenantRouter` and routes on the `k8s.namespace.name` resource
attribute:
