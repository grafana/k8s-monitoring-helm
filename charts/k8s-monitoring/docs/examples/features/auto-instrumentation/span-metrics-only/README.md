...
# Enable Auto-Instrumentation with spanMetricsOnly
# This prevents Beyla from sending full traces to the OTLP receiver
# Beyla will still generate span metrics (RED metrics) and send them to Prometheus
autoInstrumentation:
  enabled: true
  spanMetricsOnly: true  # Only collect span metrics, do not export full traces
