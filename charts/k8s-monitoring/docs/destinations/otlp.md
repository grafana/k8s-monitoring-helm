...
| processors.tailSampling.decisionCache.sampledCacheSize | integer | `0` | Configures the number of trace IDs to be kept in an LRU cache, persisting the “keep” decisions for traces that may have already been released from memory. |
| processors.tailSampling.decisionCache.notSampledCacheSize | integer | `0` | Configures number of trace IDs to be kept in an LRU cache, persisting the “drop” decisions for traces that may have already been released from memory. |
| processors.tailSampling.numTraces | integer | `50000` | Determines the buffer size of the trace delete channel which is composed of trace IDs. |
| processors.tailSampling.expectedNewTracesPerSec | integer | `0` | Determines the initial slice sizing of the current batch. |
