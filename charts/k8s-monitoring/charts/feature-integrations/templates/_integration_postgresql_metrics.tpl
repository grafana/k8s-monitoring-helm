...
{{- $dbRelabelName := printf "database_observability_postgres_%s" $alloyName }}
{{- $jobLabel := .jobLabel }}
...
discovery.relabel {{ $dbRelabelName | quote }} {
  targets = database_observability.postgres.{{ $alloyName }}.targets
  rule {
    target_label = "job"
    replacement = {{ $jobLabel | quote }}
  }
...
prometheus.scrape {{ $alloyName | quote }} {
  ...
  {{- else }}
  targets = prometheus.exporter.postgres.{{ $alloyName }}.targets
  {{- end }}
  clustering {
    enabled = true
  }
