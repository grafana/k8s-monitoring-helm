...
stage.match {
  ...
  stage.match {
    {{- if $instance.namespaces }}
    selector = `{job="integrations/grafana",instance="{{ $instance.name }}",namespace=~"{{ $instance.namespaces | join "|" }}"} |~ "^t="`
    {{- else }}
    selector = `{job="integrations/grafana",instance="{{ $instance.name }}"} |~ "^t="`
    {{- end }}
