...
| filters.annotations | object | `{}` | Drop pod logs based on pod annotations. Map of Kubernetes annotation key to the value that drops the pod's logs when matched. Set the value to `null` to drop whenever the annotation is present, regardless of its value. |
| filters.labels | object | `{}` | Drop pod logs based on pod labels. Map of Kubernetes label key to the value that drops the pod's logs when matched. Set the value to `null` to drop whenever the label is present, regardless of its value. |
