...
| filters | object | `{"annotations":{},"labels":{}}` | Drop pod logs based on the pod's annotations or labels. A pod's logs are dropped if any of the configured filters match. |
| filters.annotations | object | `{}` | Drop pod logs based on pod annotations. Map of annotation name (as configured in the `annotations`, `nodeAnnotations`, or `namespaceAnnotations` values) to the value that drops the pod's logs when matched. Set the value to `null` to drop whenever the annotation is present, regardless of its value. |
| filters.labels | object | `{}` | Drop pod logs based on pod labels. Map of label name (as configured in the `labels`, `nodeLabels`, or `namespaceLabels` values) to the value that drops the pod's logs when matched. Set the value to `null` to drop whenever the label is present, regardless of its value. |
