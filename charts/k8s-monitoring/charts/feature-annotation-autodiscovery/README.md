...
| namespaces | list | `[]` | The list of namespaces to include in autodiscovery. If empty, all namespaces are included. Entries may be exact namespace names or regular expressions (e.g. `team-.*`), and the two styles can be mixed. Regexes are fully anchored, so they must match the entire namespace name. Note: When every entry is an exact name, the discovery is much more efficient. If any entry is a regular expression, the collector must fetch objects from all namespaces, then filter them internally, which can cause performance issues on large clusters. |
