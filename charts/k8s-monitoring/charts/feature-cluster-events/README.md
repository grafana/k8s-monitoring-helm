...
|-----|------|---------|-------------|
| autoConfigureOTLPLabels | bool | `true` | Whether to automatically set the resource labels (`resources.k8s.object.kind` and `resources.k8s.object.name`) required by the Grafana Cloud Kubernetes Monitoring system when cluster events are delivered exclusively to OTLP destinations. Set to false to opt out. |
