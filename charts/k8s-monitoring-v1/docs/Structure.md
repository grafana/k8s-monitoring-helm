...
|----------------------------------------------------------------------------------------|---------------------------|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Grafana Alloy](https://grafana.com/oss/alloy-opentelemetry-collector/)                | StatefulSet               | `alloy`                       | The Grafana Alloy instance that is responsible for scraping metrics, and accepting metrics, logs, and traces via receivers.                                                                                                   |
...
| [OpenCost](https://opencost.io/)                                                       | Deployment                | `opencost`                    | Used for gathering cost metrics for the Cluster.                                                                                                                                                                              |
...
Logs is to gather logs via HostPath volume mounts. This requires this instance to be deployed as a DaemonSet. The
Grafana Alloy for Metrics is deployed as a StatefulSet, which allows it to be scaled (optionally with a
HorizontalPodAutoscaler) based on load. If it would lose its ability to scale. Also, the Grafana Alloy Singleton cannot
be scaled beyond one replica, because that would result in duplicate data being sent.
