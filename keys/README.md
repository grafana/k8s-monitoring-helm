...
-   **opencost** is signed with keyless [cosign](https://docs.sigstore.dev/).
    ...
    `charts/k8s-monitoring/charts/feature-cluster-metrics/Makefile`. That chart
    depends on the signed OCI artifact (`oci://ghcr.io/opencost/charts`) directly,
    so the cosign check covers exactly the artifact that is pulled.
